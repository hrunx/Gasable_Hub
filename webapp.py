...
async def api_mcp_tools_create(spec: ToolSpec):
    """Create a new tool module on disk and refresh discovery.

    WARNING: This endpoint writes Python code to the tools directory. It should
    only be exposed in trusted environments.
    """
    try:
        ...
        # Compute module path from spec.module if provided, else derive from name
        safe_name = "".join(ch if ch.isalnum() or ch == "_" else "_" for ch in spec.name.lower())
        mod_filename = f"{safe_name}.py"
        target = tools_dir / mod_filename
        if target.exists():
            # Overwrite to allow iteration
            pass
        with open(target, "w", encoding="utf-8") as f:
            f.write(spec.code)
        # Touch package __init__ to ensure package existence
...
@app.post("/api/assistants/sync")
async def api_assistants_sync():
    """Create or update OpenAI assistants to match DB agent settings.

    - If assistant_id is missing: create and persist it
    - If assistant_id exists: update name/instructions/model in place
    """
    try:
        client = OpenAI(api_key=os.getenv("OPENAI_API_KEY"))
        created = 0
        ...
        with get_pg_conn() as conn:
            with conn.cursor() as cur:
                ...
                rows = cur.fetchall()
                for (aid, display, prompt, model, assistant_id) in rows:
                    model_final = model or os.getenv("OPENAI_MODEL", "gpt-5-mini")
                    name_final = display or f"Gasable {aid}"
                    if assistant_id:
                        try:
                            ...
                            # If update fails (e.g., deleted remotely), recreate
                            assistant = client.beta.assistants.create(
                                model=model_final,
                                name=name_final,
                                instructions=prompt or "",
