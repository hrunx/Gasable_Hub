...
def embed_query(q: str) -> list[float]:
	...
	resp = client.embeddings.create(model=os.getenv("OPENAI_EMBED_MODEL", "text-embedding-3-small"), input=q)
	...
	with get_pg_conn() as conn:
		with conn.cursor() as cur:
			for d in docs:
				...
				if api_key:
					try:
						emb_resp = client.embeddings.create(model=os.getenv("OPENAI_EMBED_MODEL", "text-embedding-3-small"), input=text)
...
def embed_queries(queries: list[str]) -> list[list[float]]:
	client = OpenAI(api_key=os.getenv("OPENAI_API_KEY"))
	resp = client.embeddings.create(model=os.getenv("OPENAI_EMBED_MODEL", "text-embedding-3-small"), input=queries)
	# Preserve order
