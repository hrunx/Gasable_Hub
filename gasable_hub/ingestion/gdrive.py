...
def authenticate_google_drive():
    ...
    Priority:
    1) Service Account credentials (non-interactive; ideal for Cloud Run)
       - GDRIVE_SERVICE_ACCOUNT_FILE
       - GOOGLE_APPLICATION_CREDENTIALS
       - service_acc(gasablehub).json (workspace default if present)
       - GDRIVE_SERVICE_ACCOUNT_JSON (inline JSON)
    2) OAuth flow for local development via InstalledAppFlow
    """

    # 1) Try service account file paths
    candidate_paths = [
        os.getenv("GDRIVE_SERVICE_ACCOUNT_FILE"),
        os.getenv("GOOGLE_APPLICATION_CREDENTIALS"),
        "service_acc(gasablehub).json",
    ]
    ...
    # 2) Fallback to OAuth local flow (dev only)
